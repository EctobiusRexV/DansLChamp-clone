...
public class Circuit {
    ...
    private void trouverDDPBranchesParalleles() {
        for (SousCircuit sousCircuit : sousCircuits){
            for (Composant c : sousCircuit.getComposants()){
                c.voltage.setValeur(c.reactance.getValeur() * c.courant.getValeur(), Composant.Unite.UNITE);
            }
        }
    }
    ...
    private void trouverCourantBranchesParalleles() {
        ...
        for (SousCircuit sousCircuit : sousCircuits){
            sousCircuit.courant.setValeur(sousCircuit.voltage.getValeur() / sousCircuit.reactance.getValeur(), Composant.Unite.UNITE);
            for (Composant c : sousCircuit.getComposants()){
                c.courant.setValeur(sousCircuit.courant.getValeur(), Composant.Unite.UNITE);
            }
        }
    }
...
//                actuel.voltage.setValeur(circuit.get(i - 1).voltage.getValeur(), Composant.Unite.UNITE);
                ...
                if (!(circuit.get(i - 1) instanceof SousCircuit)){
                    actuel.voltage.setValeur(actuel.courant.getValeur() * ((SousCircuit) actuel).getResistanceEquiSousCircuits(), Composant.Unite.UNITE);
                } else {
                    actuel.voltage.setValeur(circuit.get(i - 1).voltage.getValeur(), Composant.Unite.UNITE);
                }
